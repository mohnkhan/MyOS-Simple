...
//  uptime), the cooperative process model (ps/run/kill/suspend/resume), and
//  user-defined command aliases (alias/unalias) — 20 commands total. Low-level
//  inb/outb are inline asm; all string handling is hand-written (no libc).
// ============================================================================
#include "keyboard.h"
#include "process.h"
#include "rtc.h"
...
void cmd_help() {
    ...
    println("", WHITE_ON_BLACK);
    println("Command Aliases:", CYAN_ON_BLACK);
    println("  alias    - Create or list command aliases", WHITE_ON_BLACK);
    println("           alias <name>=<command> - Create alias", WHITE_ON_BLACK);
    println("           alias                  - List all aliases", WHITE_ON_BLACK);
    println("  unalias  - Remove an alias (unalias <name>)", WHITE_ON_BLACK);
    println("", WHITE_ON_BLACK);
...
// Command: alias - Create or list command aliases
void cmd_alias(char* args) {
    if (args[0] == '\0') {
        // List all aliases
        if (aliases.count == 0) {
            println("No aliases defined", YELLOW_ON_BLACK);
            return;
        }
        
        println("Defined aliases:", CYAN_ON_BLACK);
        for (int i = 0; i < aliases.count; i++) {
            print("  ", WHITE_ON_BLACK);
            print(aliases.aliases[i].name, YELLOW_ON_BLACK);
            print(" = ", WHITE_ON_BLACK);
            println(aliases.aliases[i].command, GREEN_ON_BLACK);
        }
        
        char count_str[10];
        print("Total: ", CYAN_ON_BLACK);
        itoa(aliases.count, count_str);
        print(count_str, YELLOW_ON_BLACK);
        print(" aliases (max ", WHITE_ON_BLACK);
        itoa(MAX_ALIASES, count_str);
        print(count_str, YELLOW_ON_BLACK);
        println(")", WHITE_ON_BLACK);
    } else {
        ...
        // Extract alias name
        while (args[i] && args[i] != '=' && j < MAX_ALIAS_NAME - 1) {
            name[j++] = args[i++];
        }
        name[j] = '\0';
        
        if (args[i] != '=') {
            println("Invalid alias syntax. Use: alias name=command", RED_ON_BLACK);
            return;
        }
        ...
        // Extract command
        j = 0;
        while (args[i] && j < MAX_ALIAS_CMD - 1) {
            command[j++] = args[i++];
        }
        command[j] = '\0';
        
        if (strlen(name) == 0 || strlen(command) == 0) {
            println("Invalid alias: name and command cannot be empty", RED_ON_BLACK);
            return;
        }
        
        // Check if alias name conflicts with built-in commands
        for (int k = 0; k < num_commands; k++) {
            if (strcmp(name, available_commands[k]) == 0) {
                print("Cannot create alias '", RED_ON_BLACK);
                print(name, RED_ON_BLACK);
                println("': conflicts with built-in command", RED_ON_BLACK);
                return;
            }
        }
        ...
        for (int k = 0; k < aliases.count; k++) {
            if (strcmp(aliases.aliases[k].name, name) == 0) {
                found = k;
                break;
            }
        }
        ...
        if (found >= 0) {
            ...
            strcpy(aliases.aliases[found].command, command);
            print("Alias '", GREEN_ON_BLACK);
            print(name, YELLOW_ON_BLACK);
            println("' updated", GREEN_ON_BLACK);
        } else {
            // Add new alias
            if (aliases.count >= MAX_ALIASES) {
                println("Maximum number of aliases reached", RED_ON_BLACK);
                return;
            }
            
            strcpy(aliases.aliases[aliases.count].name, name);
            strcpy(aliases.aliases[aliases.count].command, command);
            aliases.count++;
            
            print("Alias '", GREEN_ON_BLACK);
            print(name, YELLOW_ON_BLACK);
            println("' created", GREEN_ON_BLACK);
        }
    }
}
...
void cmd_unalias(char* args) {
    if (args[0] == '\0') {
        println("Usage: unalias <name>", CYAN_ON_BLACK);
        return;
    }
    
    int found = -1;
    for (int i = 0; i < aliases.count; i++) {
        if (strcmp(aliases.aliases[i].name, args) == 0) {
            found = i;
            break;
        }
    }
    ...
    if (found < 0) {
        print("Alias '", RED_ON_BLACK);
        print(args, RED_ON_BLACK);
        println("' not found", RED_ON_BLACK);
        return;
    }
    
    // Remove alias by shifting remaining aliases
    for (int i = found; i < aliases.count - 1; i++) {
        strcpy(aliases.aliases[i].name, aliases.aliases[i + 1].name);
        strcpy(aliases.aliases[i].command, aliases.aliases[i + 1].command);
    }
    aliases.count--;
    
    print("Alias '", GREEN_ON_BLACK);
    print(args, YELLOW_ON_BLACK);
    println("' removed", GREEN_ON_BLACK);
}

// Resolve alias to its command
char* resolve_alias(const char* cmd) {
    for (int i = 0; i < aliases.count; i++) {
        if (strcmp(aliases.aliases[i].name, cmd) == 0) {
            return aliases.aliases[i].command;
        }
    }
    return 0;
}
...
void shell_main() {
    ...
    while (1) {
        ...
        if (alias_cmd) {
            ...
            char alias_full[MAX_CMD_LEN];
            strcpy(alias_full, alias_cmd);
            
            // Append arguments if any
            if (args[0] != '\0') {
                int len = strlen(alias_full);
                alias_full[len] = ' ';
                strcpy(alias_full + len + 1, args);
            }
            
            // Re-parse with the resolved alias
            parse_command(alias_full, cmd, args);
        }
